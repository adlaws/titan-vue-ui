...
const pJS = function(canvas, params)
{
    ...
    pJS.fn.particlesUpdate = function()
    {
        for(let i = 0; i < pJS.particles.array.length; i++)
        {
            ...
            {
                new_pos.x_left = p.radius;
                new_pos.x_right =  pJS.canvas.w;
                new_pos.y_top = p.radius;
                new_pos.y_bottom = pJS.canvas.h;
            }
            else
            {
                new_pos.x_left = -p.radius;
                new_pos.x_right = pJS.canvas.w + p.radius;
                new_pos.y_top = -p.radius;
                new_pos.y_bottom = pJS.canvas.h + p.radius;
            }
    ...
    pJS.fn.vendors.checkBeforeDraw = function()
    {
        ...
        if(pJS.particles.shape.type === 'image')
        {
            if(pJS.tmp.img_type === 'svg' && pJS.tmp.source_svg === undefined)
            {
                pJS.tmp.checkAnimFrame = window.requestAnimFrame(pJS.fn.vendors.draw);
            }
            else
            {
...
window.particlesJS = function(tag_id, params)
{
    ...
    if(tag_id instanceof Element || tag_id instanceof HTMLDocument)
    {
        pJS_tag = tag_id;
    }
    else
    ...
    if(canvas !== null)
    {
        window.pJSDom.push(new pJS(canvas, params));
    }
};
